...
clf
plot3(x1,x2,y,'k.','markersize',20);
grid on
hold on
xlabel('x1 = exam');
ylabel('x2 = agriculture');
zlabel('y = fertility');

% part two: regression plane
pause
...
h = 20;
x1val = linspace(0,50,h);
x2val = linspace(0,100,h);
[x1mesh,x2mesh] = meshgrid(x1val,x2val);
x1mesh = reshape(x1mesh,h*h,1);
x2mesh = reshape(x2mesh,h*h,1);
Xmesh = [ones(h*h,1), x1mesh, x2mesh];

yfitmesh = Xmesh*b;
yfitmesh = reshape(yfitmesh,h,h);
surf(x1val,x2val,yfitmesh);

% part three: residuals
pause

yfitdata = X*b;
residual = y-yfitdata;
plot3(x1,x2,yfitdata,'r.','markersize',10);
for i=1:length(residual)
    plot3([x1(i) x1(i)], [x2(i) x2(i)], [yfitdata(i) y(i)], 'r-');
end
